...
impl StreamingRender {
    ...
    pub(super) fn append(
        ...
        if self.stable_source_len > 0
            && crate::markdown::unwrap_markdown_fences(pending_source).as_ref() != pending_source
        {
            // Fence normalization is defined against the complete response. Applying it to an
            // incremental suffix can mistake a nested Markdown envelope for the whole document
            // and permanently shift rendered-line boundaries. Recompute once from full source
            // when a completed suffix first becomes eligible for normalization.
            self.recompute(
                raw_source,
                width,
                cwd,
