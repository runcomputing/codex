...
# Refuse prereleases by default: a `<alpha>-patched` release would otherwise become
# `releases/latest` (what install.sh pulls) since gh creates non-prereleases. Set
# ALLOW_PRERELEASE=1 to build one anyway — it's then marked --prerelease so it stays
# out of `latest`.
PRERELEASE=0
...
if [ "$PRERELEASE" = 1 ] && [ "${ALLOW_PRERELEASE:-0}" != "1" ]; then
  die "$UPSTREAM_TAG is a prerelease — refusing so it can't become releases/latest (install.sh pulls that). We ship stable tags. Re-run with ALLOW_PRERELEASE=1 to build it anyway (marked --prerelease)."
fi
...
ASSET="codex-$TARGET"
step "host $(uname -s)/$(uname -m) → $TARGET → $ASSET → release $PATCHED_TAG"
...
# ---- build + strip ----------------------------------------------------------
step "cargo build --release (-p codex-cli --bin codex) for $TARGET"
( cd "$WT/codex-rs" && cargo build --release --target "$TARGET" -p codex-cli --bin codex )
...
if [ "$HOST_OS" = "linux" ]; then strip --strip-debug --strip-unneeded "$BIN"; else strip -S -x "$BIN" 2>/dev/null || strip -S "$BIN" 2>/dev/null || true; fi
ok "built + stripped → $(du -h "$BIN" | cut -f1)"
...
if ! gh release view "$PATCHED_TAG" -R "$REPO" >/dev/null 2>&1; then
  gh release create "$PATCHED_TAG" -R "$REPO" --title "$PATCHED_TAG" \
    $([ "$PRERELEASE" = "1" ] && printf -- '--prerelease') \
    --notes "codex $UPSTREAM_TAG with the runcomputing talk-channel patch (extras/codex-channel.patch). Built per-platform via extras/release.sh." >/dev/null
  ok "created release $PATCHED_TAG"
fi
STAGE="$RUNNER_TEMP/$ASSET"; cp "$BIN" "$STAGE"; chmod +x "$STAGE"
gh release upload "$PATCHED_TAG" "$STAGE" -R "$REPO" --clobber
ok "uploaded $ASSET"

step "done → https://github.com/$REPO/releases/download/$PATCHED_TAG/$ASSET"
