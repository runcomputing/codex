...
codex + MCP channel support) for a new **stable** upstream release. You can run this
from anywhere — if you're not already in a `runcomputing/codex` checkout, **start at
step 0**; it gets you there.
...
prereleases anyway (an `…alpha-patched` release would wrongly become `releases/latest`,
which `install.sh` pulls). **If nothing is newer than what's already published, stop —
there's nothing to build** (compare against
`gh release list -R runcomputing/codex`).

## 3. Build + publish both platforms

```bash
extras/release.sh        <tag>     # macOS arm64  → codex-aarch64-apple-darwin
extras/release-linux.sh  <tag>     # linux x86_64 → codex-x86_64-unknown-linux-musl
```
...
# expect BOTH: codex-aarch64-apple-darwin  AND  codex-x86_64-unknown-linux-musl

gh api repos/runcomputing/codex/releases/latest --jq '.tag_name'
# expect: <tag>-patched
```

That's the whole job — `install.sh` serves `releases/latest`, so it now hands out the
new build automatically. **No devx change needed.**
...
| `extras/release.sh` / `release-linux.sh` | build + publish, per platform |
| `extras/README.md` | Codex-desktop wiring (`CODEX_CLI_PATH`, the LaunchAgent plist) |

devx's `install.sh` is a pure consumer: it downloads the latest `*-patched` binary from
this fork's releases and holds no codex source.
