#!/usr/bin/env bash
#
# extras/release-linux.sh — produce the linux x86_64 (musl) codex binaries from a
...
# container. Publishes codex-x86_64-unknown-linux-musl plus its companion
# codex-code-mode-host-x86_64-unknown-linux-musl to the <tag>-patched release
# on the fork.
#
...
command -v gh >/dev/null 2>&1 && gh api user >/dev/null 2>&1 || { echo "gh must be authenticated (gh auth login)" >&2; exit 1; }
TOKEN="$(gh auth token)"
...
# happens in here — only the release asset upload (uses GH_TOKEN). Full parallelism:
